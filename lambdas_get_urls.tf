resource "aws_lambda_function" "lambda_get_urls" {
  filename      = "${path.module}/lambdas/lambda_get_urls_function_payload.zip"
  function_name = "${var.tag_entity}-${var.tag_project}-get-urls"
  role          = aws_iam_role.iam_for_lambda_crawler.arn
  handler       = "get_urls.init"

  source_code_hash = "${filebase64sha256("${path.module}/lambdas/lambda_get_urls_function_payload.zip")}"
...
resource "aws_cloudwatch_log_group" "lambda_get_urls_log_group" {
  name = "/aws/lambda/${var.tag_entity}-${var.tag_project}-get-urls"
  retention_in_days = 5
  lifecycle {
